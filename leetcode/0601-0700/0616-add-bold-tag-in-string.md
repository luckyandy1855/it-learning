...
来源链接: https://leetcode.com/problems/add-bold-tag-in-string/
---
...
对于 `s` 内部的子字符串，若其存在于 `words` 数组中， 则通过添加闭合的粗体标签 `` 和 `` 进行加粗标记。
...
我们在每个子字符串之前添加，在每个子字符串之后添加: "aaabbb"。
由于前两个重叠，把它们合并得到: "aaabbb"。
由于现在这四个是连续的，把它们合并得到: "aaabbb"。
```

**提示：**

- `1 https://leetcode.cn/problems/bold-words-in-string
...
```go
// Add Bold Tag in String：按照上方思路实现主解法。
