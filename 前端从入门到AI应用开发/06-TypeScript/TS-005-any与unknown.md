...
any与unknown 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 any与unknown：
...
const result = buildRequest("请解释 any与unknown");
console.log("安全处理未知 AI API 响应", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `const data: unknown = await response.json()` | TypeScript 写法 | 本节核心语法入口 | `const data: unknown = await response.json()` |
...
- any与unknown 要结合真实数据结构使用，避免为了写类型而写类型。
...
3. [AI 场景题] 使用 any与unknown 为 AI Chat 的请求体或响应体建立类型约束。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| any与unknown | 区分 any 和 unknown 的风险边界 |
| 类型契约 | 让数据结构在开发阶段可检查 |
