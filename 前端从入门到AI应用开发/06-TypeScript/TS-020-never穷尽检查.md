...
never穷尽检查 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 never穷尽检查：
...
const result = buildRequest("请解释 never穷尽检查");
console.log("确保新增 AI 事件类型后渲染逻辑必须更新", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `const exhaustive: never = event` | TypeScript 写法 | 本节核心语法入口 | `const exhaustive: never = event` |
...
- never穷尽检查 要结合真实数据结构使用，避免为了写类型而写类型。
...
确保新增 AI 事件类型后渲染逻辑必须更新。

```ts
type AiTypeTask = {
  topic: "never穷尽检查";
  syntax: "const exhaustive: never = event";
  scene: string;
};
...
3. [AI 场景题] 使用 never穷尽检查 为 AI Chat 的请求体或响应体建立类型约束。
