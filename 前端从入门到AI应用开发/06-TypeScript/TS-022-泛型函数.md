...
泛型函数 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 泛型函数：
...
const result = buildRequest("请解释 泛型函数");
console.log("封装 parseJson<T>()", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `async function parseJson<T>(response: Response): Promise<T>` | TypeScript 写法 | 本节核心语法入口 | `async function parseJson<T>(response: Response): Promise<T>` |
...
type AiTypeTask = {
  topic: "泛型函数";
  syntax: "async function parseJson<T>(response: Response): Promise<T>";
  scene: string;
};
...
3. [AI 场景题] 使用 泛型函数 为 AI Chat 的请求体或响应体建立类型约束。
