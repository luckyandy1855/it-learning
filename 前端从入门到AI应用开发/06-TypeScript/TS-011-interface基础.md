...
interface基础 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 interface基础：
...
const result = buildRequest("请解释 interface基础");
console.log("定义 Message、ModelConfig 和 ChatSession", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `interface Message { role: Role; content: string }` | TypeScript 写法 | 本节核心语法入口 | `interface Message { role: Role; content: string }` |
...
- interface基础 要结合真实数据结构使用，避免为了写类型而写类型。
...
定义 Message、ModelConfig 和 ChatSession。

```ts
type AiTypeTask = {
  topic: "interface基础";
  syntax: "interface Message { role: Role; content: string }";
  scene: string;
};
...
const task: AiTypeTask = {
  topic: "interface基础",
  syntax: "interface Message { role: Role; content: string }",
  scene: "定义 Message、ModelConfig 和 ChatSession"
};
```
...
3. [AI 场景题] 使用 interface基础 为 AI Chat 的请求体或响应体建立类型约束。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| interface基础 | 掌握 interface 描述对象结构 |
| 类型契约 | 让数据结构在开发阶段可检查 |
