...
- 掌握 string、number、boolean 等基础类型。
...
基本类型 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 基本类型：
...
const result = buildRequest("请解释 基本类型");
console.log("约束 Prompt、temperature、stream 等字段", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `const prompt: string = "你好"` | TypeScript 写法 | 本节核心语法入口 | `const prompt: string = "你好"` |
...
约束 Prompt、temperature、stream 等字段。

```ts
type AiTypeTask = {
  topic: "基本类型";
  syntax: "const prompt: string = "你好"";
  scene: string;
};
...
const task: AiTypeTask = {
  topic: "基本类型",
  syntax: "const prompt: string = "你好"",
  scene: "约束 Prompt、temperature、stream 等字段"
};
```
...
3. [AI 场景题] 使用 基本类型 为 AI Chat 的请求体或响应体建立类型约束。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 基本类型 | 掌握 string、number、boolean 等基础类型 |
| 类型契约 | 让数据结构在开发阶段可检查 |
