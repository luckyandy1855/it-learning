...
type别名 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 type别名：
...
const result = buildRequest("请解释 type别名");
console.log("定义 Role、RequestStatus 和回调类型", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `type Role = "user" | "assistant"` | TypeScript 写法 | 本节核心语法入口 | `type Role = "user" | "assistant"` |
...
- type别名 要结合真实数据结构使用，避免为了写类型而写类型。
...
定义 Role、RequestStatus 和回调类型。

```ts
type AiTypeTask = {
  topic: "type别名";
  syntax: "type Role = "user" | "assistant"";
  scene: string;
};
...
const task: AiTypeTask = {
  topic: "type别名",
  syntax: "type Role = "user" | "assistant"",
  scene: "定义 Role、RequestStatus 和回调类型"
};
```
...
3. [AI 场景题] 使用 type别名 为 AI Chat 的请求体或响应体建立类型约束。
