...
泛型接口 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
```ts
interface PageResult<T> { items: T[] }
```
...
下面用一个类型安全的 AI Chat 请求构建示例演示 泛型接口：
...
const result = buildRequest("请解释 泛型接口");
console.log("定义分页响应、流式事件和缓存结果", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `interface PageResult<T> { items: T[] }` | TypeScript 写法 | 本节核心语法入口 | `interface PageResult<T> { items: T[] }` |
...
type AiTypeTask = {
  topic: "泛型接口";
  syntax: "interface PageResult<T> { items: T[] }";
  scene: string;
};
...
3. [AI 场景题] 使用 泛型接口 为 AI Chat 的请求体或响应体建立类型约束。
