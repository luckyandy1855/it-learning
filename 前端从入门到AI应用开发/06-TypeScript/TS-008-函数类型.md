...
函数类型 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
```ts
function sendPrompt(prompt: string): Promise<void>
```
...
下面用一个类型安全的 AI Chat 请求构建示例演示 函数类型：
...
const result = buildRequest("请解释 函数类型");
console.log("约束 sendPrompt、renderMessage 等函数", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `function sendPrompt(prompt: string): Promise<void>` | TypeScript 写法 | 本节核心语法入口 | `function sendPrompt(prompt: string): Promise<void>` |
...
约束 sendPrompt、renderMessage 等函数。

```ts
type AiTypeTask = {
  topic: "函数类型";
  syntax: "function sendPrompt(prompt: string): Promise<void>";
  scene: string;
};
...
const task: AiTypeTask = {
  topic: "函数类型",
  syntax: "function sendPrompt(prompt: string): Promise<void>",
  scene: "约束 sendPrompt、renderMessage 等函数"
};
```
...
3. [AI 场景题] 使用 函数类型 为 AI Chat 的请求体或响应体建立类型约束。
