...
只读属性 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 只读属性：
...
const result = buildRequest("请解释 只读属性");
console.log("保护消息 id、创建时间和模型能力声明", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `readonly id: string` | TypeScript 写法 | 本节核心语法入口 | `readonly id: string` |
...
3. [AI 场景题] 使用 只读属性 为 AI Chat 的请求体或响应体建立类型约束。
