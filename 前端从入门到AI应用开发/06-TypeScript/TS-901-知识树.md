...
- 整理 TypeScript 从基础类型到 AI API 建模的学习路线。
...
知识树 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 知识树：
...
const result = buildRequest("请解释 知识树");
console.log("把类型系统、泛型、工具类型和 React+TS 串成路线图", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `const roadmap: string[] = []` | TypeScript 写法 | 本节核心语法入口 | `const roadmap: string[] = []` |
...
把类型系统、泛型、工具类型和 React+TS 串成路线图。

```ts
type AiTypeTask = {
  topic: "知识树";
  syntax: "const roadmap: string[] = []";
  scene: string;
};
...
const task: AiTypeTask = {
  topic: "知识树",
  syntax: "const roadmap: string[] = []",
  scene: "把类型系统、泛型、工具类型和 React+TS 串成路线图"
};
```
...
3. [AI 场景题] 使用 知识树 为 AI Chat 的请求体或响应体建立类型约束。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 知识树 | 整理 TypeScript 从基础类型到 AI API 建模的学习路线 |
| 类型契约 | 让数据结构在开发阶段可检查 |
