...
类型扩展 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
```ts
type ToolMessage = Message & { toolName: string }
```
...
下面用一个类型安全的 AI Chat 请求构建示例演示 类型扩展：
...
const result = buildRequest("请解释 类型扩展");
console.log("扩展基础消息为工具消息、引用消息", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `type ToolMessage = Message & { toolName: string }` | TypeScript 写法 | 本节核心语法入口 | `type ToolMessage = Message & { toolName: string }` |
...
type AiTypeTask = {
  topic: "类型扩展";
  syntax: "type ToolMessage = Message & { toolName: string }";
  scene: string;
};
...
3. [AI 场景题] 使用 类型扩展 为 AI Chat 的请求体或响应体建立类型约束。
