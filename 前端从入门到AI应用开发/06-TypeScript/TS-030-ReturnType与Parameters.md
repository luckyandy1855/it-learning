...
## 📖 什么是ReturnType与Parameters

ReturnType与Parameters 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
const message: Message = {
  role: "user",
  content: "解释 ReturnType与Parameters"
};
```
...
下面用一个类型安全的 AI Chat 请求构建示例演示 ReturnType与Parameters：
...
const result = buildRequest("请解释 ReturnType与Parameters");
console.log("复用 sendPrompt 的请求参数类型", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `ReturnType<typeof createMessage>` | TypeScript 写法 | 本节核心语法入口 | `ReturnType<typeof createMessage>` |
...
- ReturnType与Parameters 要结合真实数据结构使用，避免为了写类型而写类型。
...
复用 sendPrompt 的请求参数类型。

```ts
type AiTypeTask = {
  topic: "ReturnType与Parameters";
  syntax: "ReturnType<typeof createMessage>";
  scene: string;
};
...
3. [AI 场景题] 使用 ReturnType与Parameters 为 AI Chat 的请求体或响应体建立类型约束。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| ReturnType与Parameters | 从函数中提取返回值和参数类型 |
| 类型契约 | 让数据结构在开发阶段可检查 |
