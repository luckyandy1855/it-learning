...
类型缩窄 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 类型缩窄：
...
const result = buildRequest("请解释 类型缩窄");
console.log("安全读取不同 AI 消息类型的字段", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `if (message.role === "assistant")` | TypeScript 写法 | 本节核心语法入口 | `if (message.role === "assistant")` |
...
type AiTypeTask = {
  topic: "类型缩窄";
  syntax: "if (message.role === "assistant")";
  scene: string;
};
...
3. [AI 场景题] 使用 类型缩窄 为 AI Chat 的请求体或响应体建立类型约束。
