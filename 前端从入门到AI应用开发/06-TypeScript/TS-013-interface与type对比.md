...
interface与type对比 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
```ts
interface ModelConfig extends BaseConfig {}
```
...
下面用一个类型安全的 AI Chat 请求构建示例演示 interface与type对比：
...
const result = buildRequest("请解释 interface与type对比");
console.log("在 AI 项目中选择可扩展接口或组合类型", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `interface ModelConfig extends BaseConfig {}` | TypeScript 写法 | 本节核心语法入口 | `interface ModelConfig extends BaseConfig {}` |
...
- interface与type对比 要结合真实数据结构使用，避免为了写类型而写类型。
...
在 AI 项目中选择可扩展接口或组合类型。

```ts
type AiTypeTask = {
  topic: "interface与type对比";
  syntax: "interface ModelConfig extends BaseConfig {}";
  scene: string;
};
...
3. [AI 场景题] 使用 interface与type对比 为 AI Chat 的请求体或响应体建立类型约束。
