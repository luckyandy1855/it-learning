...
useRef类型 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 useRef类型：
...
const result = buildRequest("请解释 useRef类型");
console.log("自动聚焦 Prompt 输入框和滚动到底部", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `useRef<HTMLTextAreaElement | null>(null)` | TypeScript 写法 | 本节核心语法入口 | `useRef<HTMLTextAreaElement | null>(null)` |
...
- useRef类型 要结合真实数据结构使用，避免为了写类型而写类型。
...
自动聚焦 Prompt 输入框和滚动到底部。

```ts
type AiTypeTask = {
  topic: "useRef类型";
  syntax: "useRef<HTMLTextAreaElement | null>(null)";
  scene: string;
};
...
3. [AI 场景题] 使用 useRef类型 为 AI Chat 的请求体或响应体建立类型约束。
