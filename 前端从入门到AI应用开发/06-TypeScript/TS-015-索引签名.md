...
索引签名 是 TypeScript 类型系统中的重要知识点。它帮助我们在代码运行前发现数据结构不一致、字段拼写错误、参数类型错误和接口响应误用。
...
下面用一个类型安全的 AI Chat 请求构建示例演示 索引签名：
...
const result = buildRequest("请解释 索引签名");
console.log("描述模型参数、headers 和 metadata", result);
```
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `type Metadata = { [key: string]: string }` | TypeScript 写法 | 本节核心语法入口 | `type Metadata = { [key: string]: string }` |
...
描述模型参数、headers 和 metadata。

```ts
type AiTypeTask = {
  topic: "索引签名";
  syntax: "type Metadata = { [key: string]: string }";
  scene: string;
};
...
const task: AiTypeTask = {
  topic: "索引签名",
  syntax: "type Metadata = { [key: string]: string }",
  scene: "描述模型参数、headers 和 metadata"
};
```
...
3. [AI 场景题] 使用 索引签名 为 AI Chat 的请求体或响应体建立类型约束。
