...
Props基础 是 React 开发中需要稳定掌握的核心知识点之一。React 的核心思想是用组件描述界面，用状态驱动变化，再通过数据流把复杂界面拆成可维护的小块。

在 AI 应用中，界面变化非常频繁：用户输入 Prompt、切换模型、等待回复、接收流式 token、展示错误、保存历史。Props基础 可以帮助我们把这些变化组织成清晰的组件逻辑。
...
Props基础 的作用，是让这条链路中的某一环更清晰、更可控或更高效。
...
下面用一个 AI Chat 教学组件演示 Props基础 的使用方式：
...
export default function Demo() {
  ...
  const [messages, setMessages] = useState([
    { id: "m1", role: "assistant", content: "这里演示 Props基础。" }
  ]);
  ...
  return (
    <section>
      <h1>Props基础</h1>
      <p>把 role、content 和 createdAt 传给消息组件</p>
      <p>核心写法：<code><Message role="user" content={prompt} /></code></p>
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `<Message role="user" content={prompt} />` | React 写法 | 本节核心入口 | `<Message role="user" content={prompt} />` |
...
- Props基础 要放到真实交互流程中理解，才能避免只会写示例不会做项目。
...
把 role、content 和 createdAt 传给消息组件。下面的片段展示如何把本节知识落到 AI Chat 数据流中：

```jsx
const chatTask = {
  topic: "Props基础",
  syntax: "<Message role="user" content={prompt} />",
  scene: "把 role、content 和 createdAt 传给消息组件"
};
...
1. [基础题] 写一个最小 React 组件，页面中显示“学习 Props基础”。
2. [进阶题] 把本节示例拆成 `MessageList` 和 `PromptInput` 两个组件。
3. [AI 场景题] 在 AI Chat 页面中使用 Props基础 完成一次 Prompt 输入、发送或渲染流程。
