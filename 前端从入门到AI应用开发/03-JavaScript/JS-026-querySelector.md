# querySelector（JS-026）

## 🎯 本节学习目标

- 理解 querySelector 在 JavaScript 中解决什么问题。
- 掌握 `document.querySelector("#promptInput")` 这类核心写法的基本使用方式。
- 能够把 querySelector 应用到 AI 聊天、Prompt 工具或 Agent 前端界面中。
...
本节的重点不是背语法，而是理解它在真实前端代码里承担的职责。从页面中获取输入框、按钮和消息容器，这正是 AI Web 应用中经常出现的需求。

掌握 querySelector 后，你会更容易阅读后续的 DOM 操作、接口请求、异步流程和流式输出代码。
...
```html
<script>
  const prompt = "请解释 querySelector";
  const topic = "querySelector";
...
下面用一个 AI 学习面板演示 querySelector 的基本用法：
...
<body>
  ...
  <script>
    ...
    const messages = [
      ...
      { role: "user", content: "请解释 querySelector。" }
    ];

    function runDemo(prompt) {
      const normalizedPrompt = prompt.trim() || "请用 AI 场景解释 querySelector";
      const result = {
        topic: "querySelector",
        usage: "从页面中获取输入框、按钮和消息容器",
        syntax: "document.querySelector("#promptInput")",
...
页面会显示一个输入框、一个按钮和一个输出区。输入 Prompt 并点击按钮后，输出区会展示与 querySelector 相关的结构化结果。
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `document.querySelector("#promptInput")` | 语法/API | 本节核心写法 | `document.querySelector("#promptInput")` |
| `const` | 关键字 | 声明不重新赋值的绑定 | `const topic = "querySelector"` |
...
- querySelector 的价值在于让代码更清晰地表达业务意图。
...
- 易错点1：只记住 `document.querySelector("#promptInput")` 的形式，却不知道它解决什么问题。正确写法：先明确输入、处理和输出。
...
从页面中获取输入框、按钮和消息容器。下面是一个可迁移到 AI 前端项目中的小片段：

```js
const aiTask = {
  topic: "querySelector",
  syntax: "document.querySelector("#promptInput")",
  prompt: "请基于真实业务场景解释 querySelector"
};
...
1. [基础题] 写出 `document.querySelector("#promptInput")` 的一个最小示例，并在 Console 中输出结果。
2. [进阶题] 把本节示例改造成一个可复用函数，输入 Prompt，返回结构化对象。
3. [AI 场景题] 在 AI 聊天界面中使用 querySelector 处理一组消息或一次请求配置。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| querySelector | 从页面中获取输入框、按钮和消息容器 |
| 核心写法 | `document.querySelector("#promptInput")` 是本节需要熟悉的入口 |
