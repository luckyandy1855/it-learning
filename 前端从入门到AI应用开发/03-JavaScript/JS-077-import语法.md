...
- 理解 import语法 在 JavaScript 中解决什么问题。
- 掌握 `import { sendPrompt } from "./api.js"` 这类核心写法的基本使用方式。
- 能够把 import语法 应用到 AI 聊天、Prompt 工具或 Agent 前端界面中。

---

## 📖 什么是import语法

ES Module 是浏览器原生模块系统。

本节的重点不是背语法，而是理解它在真实前端代码里承担的职责。从其他文件导入请求函数、渲染函数和配置，这正是 AI Web 应用中经常出现的需求。

掌握 import语法 后，你会更容易阅读后续的 DOM 操作、接口请求、异步流程和流式输出代码。

---

## 🧠 原理讲解

它让 AI Web 项目能把请求、渲染、状态和配置拆成清晰文件。

可以把 import语法 放进下面这条主线理解：
...
下面用一个 AI 学习面板演示 import语法 的基本用法：
...
<body>
  ...
  <script>
    ...
    const messages = [
      ...
      { role: "user", content: "请解释 import语法。" }
    ];

    function runDemo(prompt) {
      const normalizedPrompt = prompt.trim() || "请用 AI 场景解释 import语法";
      const result = {
        topic: "import语法",
        usage: "从其他文件导入请求函数、渲染函数和配置",
        syntax: "import { sendPrompt } from "./api.js"",
...
页面会显示一个输入框、一个按钮和一个输出区。输入 Prompt 并点击按钮后，输出区会展示与 import语法 相关的结构化结果。
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `import { sendPrompt } from "./api.js"` | 语法/API | 本节核心写法 | `import { sendPrompt } from "./api.js"` |
| `const` | 关键字 | 声明不重新赋值的绑定 | `const topic = "import"` |
...
- 易错点1：只记住 `import { sendPrompt } from "./api.js"` 的形式，却不知道它解决什么问题。正确写法：先明确输入、处理和输出。
...
从其他文件导入请求函数、渲染函数和配置。下面是一个可迁移到 AI 前端项目中的小片段：

```js
const aiTask = {
  topic: "import语法",
  syntax: "import { sendPrompt } from "./api.js"",
  prompt: "请基于真实业务场景解释 import语法"
};
...
1. [基础题] 写出 `import { sendPrompt } from "./api.js"` 的一个最小示例，并在 Console 中输出结果。
2. [进阶题] 把本节示例改造成一个可复用函数，输入 Prompt，返回结构化对象。
3. [AI 场景题] 在 AI 聊天界面中使用 import语法 处理一组消息或一次请求配置。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| import语法 | 从其他文件导入请求函数、渲染函数和配置 |
| 核心写法 | `import { sendPrompt } from "./api.js"` 是本节需要熟悉的入口 |
