# ReadableStream（JS-073）

## 🎯 本节学习目标

- 理解 ReadableStream 在 JavaScript 中解决什么问题。
- 掌握 `response.body.getReader()` 这类核心写法的基本使用方式。
- 能够把 ReadableStream 应用到 AI 聊天、Prompt 工具或 Agent 前端界面中。

---

## 📖 什么是ReadableStream

流式通信让前端逐步接收 AI 回复，而不是等待完整结果。

本节的重点不是背语法，而是理解它在真实前端代码里承担的职责。用 fetch 读取流式响应体，这正是 AI Web 应用中经常出现的需求。

掌握 ReadableStream 后，你会更容易阅读后续的 DOM 操作、接口请求、异步流程和流式输出代码。

---

## 🧠 原理讲解

SSE、ReadableStream 和 WebSocket 是实现实时体验的重要基础。

可以把 ReadableStream 放进下面这条主线理解：
...
```html
<script>
  const prompt = "请解释 ReadableStream";
  const topic = "ReadableStream";
...
下面用一个 AI 学习面板演示 ReadableStream 的基本用法：
...
<body>
  ...
  <script>
    ...
    const messages = [
      ...
      { role: "user", content: "请解释 ReadableStream。" }
    ];

    function runDemo(prompt) {
      const normalizedPrompt = prompt.trim() || "请用 AI 场景解释 ReadableStream";
      const result = {
        topic: "ReadableStream",
        usage: "用 fetch 读取流式响应体",
        syntax: "response.body.getReader()",
...
页面会显示一个输入框、一个按钮和一个输出区。输入 Prompt 并点击按钮后，输出区会展示与 ReadableStream 相关的结构化结果。
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `response.body.getReader()` | 语法/API | 本节核心写法 | `response.body.getReader()` |
| `const` | 关键字 | 声明不重新赋值的绑定 | `const topic = "ReadableStream"` |
...
- ReadableStream 的价值在于让代码更清晰地表达业务意图。
...
- 易错点1：只记住 `response.body.getReader()` 的形式，却不知道它解决什么问题。正确写法：先明确输入、处理和输出。
...
用 fetch 读取流式响应体。下面是一个可迁移到 AI 前端项目中的小片段：

```js
const aiTask = {
  topic: "ReadableStream",
  syntax: "response.body.getReader()",
  prompt: "请基于真实业务场景解释 ReadableStream"
};
...
1. [基础题] 写出 `response.body.getReader()` 的一个最小示例，并在 Console 中输出结果。
2. [进阶题] 把本节示例改造成一个可复用函数，输入 Prompt，返回结构化对象。
3. [AI 场景题] 在 AI 聊天界面中使用 ReadableStream 处理一组消息或一次请求配置。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| ReadableStream | 用 fetch 读取流式响应体 |
| 核心写法 | `response.body.getReader()` 是本节需要熟悉的入口 |
