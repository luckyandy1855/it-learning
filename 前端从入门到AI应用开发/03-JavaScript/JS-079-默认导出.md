...
- 理解 默认导出 在 JavaScript 中解决什么问题。
- 掌握 `export default class ChatClient` 这类核心写法的基本使用方式。
- 能够把 默认导出 应用到 AI 聊天、Prompt 工具或 Agent 前端界面中。
...
本节的重点不是背语法，而是理解它在真实前端代码里承担的职责。导出一个模块的主要能力，这正是 AI Web 应用中经常出现的需求。

掌握 默认导出 后，你会更容易阅读后续的 DOM 操作、接口请求、异步流程和流式输出代码。
...
```html
<script>
  const prompt = "请解释 默认导出";
  const topic = "default export";
...
<body>
  ...
  <script>
    ...
    const messages = [
      ...
      { role: "user", content: "请解释 默认导出。" }
    ];

    function runDemo(prompt) {
      const normalizedPrompt = prompt.trim() || "请用 AI 场景解释 默认导出";
      const result = {
        topic: "默认导出",
        usage: "导出一个模块的主要能力",
        syntax: "export default class ChatClient",
...
页面会显示一个输入框、一个按钮和一个输出区。输入 Prompt 并点击按钮后，输出区会展示与 默认导出 相关的结构化结果。
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `export default class ChatClient` | 语法/API | 本节核心写法 | `export default class ChatClient` |
| `const` | 关键字 | 声明不重新赋值的绑定 | `const topic = "default export"` |
...
- 易错点1：只记住 `export default class ChatClient` 的形式，却不知道它解决什么问题。正确写法：先明确输入、处理和输出。
...
导出一个模块的主要能力。下面是一个可迁移到 AI 前端项目中的小片段：

```js
const aiTask = {
  topic: "默认导出",
  syntax: "export default class ChatClient",
  prompt: "请基于真实业务场景解释 默认导出"
};
...
1. [基础题] 写出 `export default class ChatClient` 的一个最小示例，并在 Console 中输出结果。
2. [进阶题] 把本节示例改造成一个可复用函数，输入 Prompt，返回结构化对象。
3. [AI 场景题] 在 AI 聊天界面中使用 默认导出 处理一组消息或一次请求配置。
...
|--------|-----------|
| 默认导出 | 导出一个模块的主要能力 |
| 核心写法 | `export default class ChatClient` 是本节需要熟悉的入口 |
