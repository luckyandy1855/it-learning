...
- 理解 避免UI卡顿 在 JavaScript 中解决什么问题。
- 掌握 `requestAnimationFrame(update)` 这类核心写法的基本使用方式。
- 能够把 避免UI卡顿 应用到 AI 聊天、Prompt 工具或 Agent 前端界面中。

---

## 📖 什么是避免UI卡顿

Event Loop 决定同步任务、微任务和宏任务的执行顺序。

本节的重点不是背语法，而是理解它在真实前端代码里承担的职责。把长任务拆分，避免 AI 界面失去响应，这正是 AI Web 应用中经常出现的需求。

掌握 避免UI卡顿 后，你会更容易阅读后续的 DOM 操作、接口请求、异步流程和流式输出代码。

---

## 🧠 原理讲解

理解它可以解释 AI 界面为什么会卡顿或延迟更新。

可以把 避免UI卡顿 放进下面这条主线理解：
...
```html
<script>
  const prompt = "请解释 避免UI卡顿";
  const topic = "non-blocking UI";
...
<body>
  ...
  <script>
    ...
    const messages = [
      ...
      { role: "user", content: "请解释 避免UI卡顿。" }
    ];

    function runDemo(prompt) {
      const normalizedPrompt = prompt.trim() || "请用 AI 场景解释 避免UI卡顿";
      const result = {
        topic: "避免UI卡顿",
        usage: "把长任务拆分，避免 AI 界面失去响应",
        syntax: "requestAnimationFrame(update)",
...
页面会显示一个输入框、一个按钮和一个输出区。输入 Prompt 并点击按钮后，输出区会展示与 避免UI卡顿 相关的结构化结果。
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `requestAnimationFrame(update)` | 语法/API | 本节核心写法 | `requestAnimationFrame(update)` |
| `const` | 关键字 | 声明不重新赋值的绑定 | `const topic = "non-blocking UI"` |
...
- 易错点1：只记住 `requestAnimationFrame(update)` 的形式，却不知道它解决什么问题。正确写法：先明确输入、处理和输出。
...
把长任务拆分，避免 AI 界面失去响应。下面是一个可迁移到 AI 前端项目中的小片段：

```js
const aiTask = {
  topic: "避免UI卡顿",
  syntax: "requestAnimationFrame(update)",
  prompt: "请基于真实业务场景解释 避免UI卡顿"
};
...
1. [基础题] 写出 `requestAnimationFrame(update)` 的一个最小示例，并在 Console 中输出结果。
2. [进阶题] 把本节示例改造成一个可复用函数，输入 Prompt，返回结构化对象。
3. [AI 场景题] 在 AI 聊天界面中使用 避免UI卡顿 处理一组消息或一次请求配置。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 避免UI卡顿 | 把长任务拆分，避免 AI 界面失去响应 |
| 核心写法 | `requestAnimationFrame(update)` 是本节需要熟悉的入口 |
