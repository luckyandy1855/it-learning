...
知识库RAG实现 是综合项目实战中的一个关键阶段。前面模块分别学习了 HTML、CSS、JavaScript、浏览器、React、TypeScript、工程化、AI Web 和 MCP，本模块要把这些能力串成一个可交付应用。
...
下面用一个项目里程碑结构演示 知识库RAG实现 的交付拆解方式：
...
function summarizeMilestone(item: ProjectMilestone): string {
  ...
  return `${item.name}：${doneCount}/${item.tasks.length} 已完成，核心方法是 RAG`;
}
...
3. [AI 场景题] 基于 知识库RAG实现，为“AI 知识库助手”补充一份可执行检查清单。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 知识库RAG实现 | 完成文档切分、embedding、检索和上下文注入 |
| 综合项目 | 把前面模块能力串成可交付应用 |
