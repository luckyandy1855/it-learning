...
- 接入 MCP Server，把外部工具和资源暴露给 AI Agent。
...
MCP工具集成 是综合项目实战中的一个关键阶段。前面模块分别学习了 HTML、CSS、JavaScript、浏览器、React、TypeScript、工程化、AI Web 和 MCP，本模块要把这些能力串成一个可交付应用。
...
下面用一个项目里程碑结构演示 MCP工具集成 的交付拆解方式：
...
function summarizeMilestone(item: ProjectMilestone): string {
  ...
  return `${item.name}：${doneCount}/${item.tasks.length} 已完成，核心方法是 MCP tool`;
}
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `MCP tool` | 项目交付物 | 本节核心产出 | `MCP tool` |
...
让助手读取课程目录、生成复盘和调用业务工具。

```ts
const aiProjectStage = {
  stage: "MCP工具集成",
  deliverable: "MCP tool",
  aiScenario: "让助手读取课程目录、生成复盘和调用业务工具",
  reviewRequired: true
};
```
...
3. [AI 场景题] 基于 MCP工具集成，为“AI 知识库助手”补充一份可执行检查清单。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| MCP工具集成 | 接入 MCP Server，把外部工具和资源暴露给 AI Agent |
| 综合项目 | 把前面模块能力串成可交付应用 |
