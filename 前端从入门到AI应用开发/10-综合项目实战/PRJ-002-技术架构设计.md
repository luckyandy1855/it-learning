...
技术架构设计 是综合项目实战中的一个关键阶段。前面模块分别学习了 HTML、CSS、JavaScript、浏览器、React、TypeScript、工程化、AI Web 和 MCP，本模块要把这些能力串成一个可交付应用。
...
function summarizeMilestone(item: ProjectMilestone): string {
  ...
  return `${item.name}：${doneCount}/${item.tasks.length} 已完成，核心方法是 architecture`;
}
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `architecture` | 项目交付物 | 本节核心产出 | `architecture` |
...
3. [AI 场景题] 基于 技术架构设计，为“AI 知识库助手”补充一份可执行检查清单。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 技术架构设计 | 设计前端、后端、模型服务、向量库和 MCP 的整体架构 |
| 综合项目 | 把前面模块能力串成可交付应用 |
