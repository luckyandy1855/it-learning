...
项目需求与产品定义 是综合项目实战中的一个关键阶段。前面模块分别学习了 HTML、CSS、JavaScript、浏览器、React、TypeScript、工程化、AI Web 和 MCP，本模块要把这些能力串成一个可交付应用。
...
下面用一个项目里程碑结构演示 项目需求与产品定义 的交付拆解方式：
...
function summarizeMilestone(item: ProjectMilestone): string {
  ...
  return `${item.name}：${doneCount}/${item.tasks.length} 已完成，核心方法是 PRD`;
}
...
- 项目需求与产品定义 不是孤立任务，它会影响后续实现和维护成本。
...
确定用户、核心场景、MVP 范围和验收标准。

```ts
const aiProjectStage = {
  stage: "项目需求与产品定义",
  deliverable: "PRD",
  aiScenario: "确定用户、核心场景、MVP 范围和验收标准",
  reviewRequired: true
};
```
...
3. [AI 场景题] 基于 项目需求与产品定义，为“AI 知识库助手”补充一份可执行检查清单。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 项目需求与产品定义 | 把 AI 知识库助手从想法拆成可执行需求 |
| 综合项目 | 把前面模块能力串成可交付应用 |
