...
测试与质量门禁 是综合项目实战中的一个关键阶段。前面模块分别学习了 HTML、CSS、JavaScript、浏览器、React、TypeScript、工程化、AI Web 和 MCP，本模块要把这些能力串成一个可交付应用。
...
下面用一个项目里程碑结构演示 测试与质量门禁 的交付拆解方式：
...
const milestone: ProjectMilestone = {
  name: "测试与质量门禁",
  goal: "确保 AI Chat、RAG 和 MCP 集成可回归",
  tasks: [
...
function summarizeMilestone(item: ProjectMilestone): string {
  ...
  return `${item.name}：${doneCount}/${item.tasks.length} 已完成，核心方法是 vitest`;
}
...
确保 AI Chat、RAG 和 MCP 集成可回归。

```ts
const aiProjectStage = {
  stage: "测试与质量门禁",
  deliverable: "vitest",
  aiScenario: "确保 AI Chat、RAG 和 MCP 集成可回归",
  reviewRequired: true
};
```
...
3. [AI 场景题] 基于 测试与质量门禁，为“AI 知识库助手”补充一份可执行检查清单。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 测试与质量门禁 | 为核心函数、组件、Hook 和接口状态补充测试 |
| 综合项目 | 把前面模块能力串成可交付应用 |
