...
- 搭建 Vite、React、TypeScript、Lint、测试和目录结构。
...
工程初始化 是综合项目实战中的一个关键阶段。前面模块分别学习了 HTML、CSS、JavaScript、浏览器、React、TypeScript、工程化、AI Web 和 MCP，本模块要把这些能力串成一个可交付应用。
...
function summarizeMilestone(item: ProjectMilestone): string {
  ...
  return `${item.name}：${doneCount}/${item.tasks.length} 已完成，核心方法是 pnpm create vite`;
}
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `pnpm create vite` | 项目交付物 | 本节核心产出 | `pnpm create vite` |
...
const aiProjectStage = {
  stage: "工程初始化",
  deliverable: "pnpm create vite",
  aiScenario: "建立可持续迭代的 AI 应用工程骨架",
  reviewRequired: true
};
```
...
3. [AI 场景题] 基于 工程初始化，为“AI 知识库助手”补充一份可执行检查清单。
...
| 知识点 | 一句话总结 |
|--------|-----------|
| 工程初始化 | 搭建 Vite、React、TypeScript、Lint、测试和目录结构 |
| 综合项目 | 把前面模块能力串成可交付应用 |
