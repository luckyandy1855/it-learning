...
部署上线与复盘 是综合项目实战中的一个关键阶段。前面模块分别学习了 HTML、CSS、JavaScript、浏览器、React、TypeScript、工程化、AI Web 和 MCP，本模块要把这些能力串成一个可交付应用。
...
下面用一个项目里程碑结构演示 部署上线与复盘 的交付拆解方式：
...
function summarizeMilestone(item: ProjectMilestone): string {
  ...
  return `${item.name}：${doneCount}/${item.tasks.length} 已完成，核心方法是 release checklist`;
}
...
| 属性/API | 类型 | 说明 | 示例 |
|---------|------|------|------|
| `release checklist` | 项目交付物 | 本节核心产出 | `release checklist` |
...
把 AI 应用交付到可使用、可维护的状态。

```ts
const aiProjectStage = {
  stage: "部署上线与复盘",
  deliverable: "release checklist",
  aiScenario: "把 AI 应用交付到可使用、可维护的状态",
  reviewRequired: true
};
```
...
3. [AI 场景题] 基于 部署上线与复盘，为“AI 知识库助手”补充一份可执行检查清单。
